...
│   ├── network.{h,cpp}   # WiFi 配网 (SoftAP + Captive Portal) + 上报
│   └── tts.{h,cpp}       # 语音播报 (可选)
└── README.md
```
...
3. 选择 WiFi → 输入密码 → 填后端 URL → **保存并连接**
4. OLED 右上角 WiFi 图标常亮即表示连接成功；带斜杠表示未连接

配网凭证保存在 NVS 命名空间 `plantguard`（键：`ssid` / `pass` / `serverUrl`）。
...
| 土壤一直 0% | 标定值（`SOIL_DRY_MAX` / `SOIL_WET_MIN`）需按你的探头实测调整 |
